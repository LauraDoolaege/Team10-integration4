...
export async function getLeaderboard(tripId){
  const res = await fetch(`${API_URL}/api/leaderboards/${tripId}`);
  if (!res.ok) {
...
export async function getCoupon(couponId) {
  const res = await fetch(`${API_URL}/api/coupon/${couponId}`);
  if (!res.ok) {
...
export async function redeemCoupon(couponId) {
  const res = await fetch(`${API_URL}/api/coupon/redeem/${couponId}`);
  if (!res.ok) {
    throw new Error("Failed to fetch coupon");
  }
  return await res.json();
...
export async function endTrip(tripId) {
  const res = await fetch(`${API_URL}/api/closetrip/${tripId}`);
...
export async function getTrip(tripId, playerId) {
  const res = await fetch(`${API_URL}/api/trips/${tripId}/${playerId}`);
...
export async function createTripVote(data) {
  const res = await fetch(`${API_URL}/api/trips/vote`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
