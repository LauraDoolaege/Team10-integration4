...
const redeemCoupon = async (couponId) => {
    // Update coupon status to 'redeemed' for the given coupon ID.
    await db.query(
        `
        UPDATE coupons
        SET status = 'redeemed'
...
const getCoupon = async (couponId) => {
    // Retrieve coupon information by its unique ID.
    ...
    // Fetch the trip details and associated cafe metadata for the coupon.
...
async function createCoupon(couponId, tripId) {
    // Insert a new coupon into the database, ignoring duplicates.
    await db.query(
        `
...
const getLeaderboard = async (tripId) => {
    try {
        ...
        // Fetch the initiator ID and name of the cafe for the specified trip.
        const [tripRows] = await db.query(
            `
...
const clearTripData = async (tripId) => {
    try {
        // Delete all votes associated with the trip ID.
        await db.query(`DELETE FROM votes WHERE trip_id = ?`, [tripId]);
        // Delete all voter records associated with the trip ID.
        await db.query(`DELETE FROM trip_voters WHERE trip_id = ?`, [tripId]);
        // Delete all player records linked to the trip ID.
        await db.query(`DELETE FROM trip_players WHERE trip_id = ?`, [tripId]);
        // Delete all possible dates proposed for the trip ID.
